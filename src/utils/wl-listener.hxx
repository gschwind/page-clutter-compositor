...
template<typename T /* the type of the parameter provided by the signal */>
struct wl_listener_t {
	...
	static void _call(wl_listener* _listener, void* data /* the parameter of the signal*/) {
	...
	void connect(wl_signal* signal, F f) {
		_func = f;
		_pod._func = &_func;
		_pod._listener.notify = &wl_listener_t::_call;
		disconnect();
