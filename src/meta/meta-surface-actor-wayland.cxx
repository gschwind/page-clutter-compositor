...
#include "meta-surface-actor-wayland.hxx"
...
{
   MetaSurfaceActorWaylandPrivate *priv = reinterpret_cast<MetaSurfaceActorWaylandPrivate*>(meta_surface_actor_wayland_get_instance_private (actor));
   wl::wl_surface *surface = priv->surface;
   MetaWindow *window;
   int output_scale = 1;
...
{
  wl::wl_surface *surface = meta_surface_actor_wayland_get_surface (self);
  wl::wl_buffer *buffer = surface->pending.buffer;
...
//        {
//          wl::wl_surface *subsurf = iter->data;
//
                                          ...
                                          wl::wl_output         *monitor)
{
  ...
  cairo_rectangle_int_t rect = {
	      .x = monitor->output->x,
	      .y = monitor->output->y,
	      .width = monitor->output->width,
	      .height = monitor->output->height
  };
...
{
  MetaSurfaceActorWaylandPrivate *priv = reinterpret_cast<MetaSurfaceActorWaylandPrivate *>(meta_surface_actor_wayland_get_instance_private (META_SURFACE_ACTOR_WAYLAND (actor)));
  wl::wl_surface *surface = priv->surface;
...
MetaSurfaceActor *
meta_surface_actor_wayland_new (wl::wl_surface *surface)
{
