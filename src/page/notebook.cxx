...
void notebook_t::_set_selected(view_p c) {
	/** already selected **/
	if(_selected == c)
		return;
...
void notebook_t::iconify_client(view_p x) {
	if(_selected == x) {
		_start_fading();
		_children.remove(x);
		_schedule_repaint();
...
rect notebook_t::_compute_client_size(view_p c) {
...
auto notebook_t::selected() const -> view_p {
	return _selected;
}
...
void notebook_t::activate(shared_ptr<tree_t> t) {
	...
	auto mw = dynamic_pointer_cast<view_t>(t);
	if (mw != nullptr) {
		_set_selected(mw);
	}
...
void notebook_t::_update_notebook_areas() {
	...
	if(_clients_tab_order.size() > 0) {
		...
		if(_selected != nullptr) {
			...
			_client_buttons.push_back(std::make_tuple(b, view_w{_selected}, &_theme_notebook.selected_client));
...
void notebook_t::_update_mouse_over() {
	...
	if (_allocation.is_inside(x, y)) {
		...
		tuple<rect, view_w, theme_tab_t *> * tab = nullptr;
		tuple<rect, view_w, int> * exposay = nullptr;
...
void notebook_t::_client_title_change(view_t * c) {
